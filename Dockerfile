...
- **One SystemApplication** (PHP 8.3-FPM + Nginx)
...
# Volume backup
docker run --rm -v one_system_postgres_data:/data -v $(pwd):/backup alpine tar czf /backup/postgres-backup.tar.gz -C /data .

# Volume restore
docker run --rm -v one_system_postgres_data:/data -v $(pwd):/backup alpine tar xzf /backup/postgres-backup.tar.gz -C /data
```
