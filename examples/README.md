...
| [nextjs](./nextjs) | TypeScript / Next.js 14 (App Router) | `@cas-system/nextjs-cas-client` | 9108 | `cp .env.example .env.local && (fill in values) && npm install && npm run dev` → http://localhost:9108 | npm dependency on the published `@cas-system/nextjs-cas-client@^1.0.1` (npm registry install, no local linking); `next.config.mjs` has no build-config workaround — `1.0.1` fixed the `1.0.0` `exports` map bug by shipping a proper dual ESM/CJS build, so imports resolve straight from `node_modules`. |
