...
prepare_environment() {
    ...
    if grep -q "APP_KEY=" "$ENV_FILE" && ! grep -q "APP_KEY=base64:" "$ENV_FILE"; then
        ...
        sed -i'' "s|APP_KEY=|APP_KEY=base64:$NEW_KEY|g" "$ENV_FILE"
...
wait_for_services() {
    ...
    while [ $timeout -gt 0 ]; do
        # Use the correct healthcheck command from your docker-compose file
    ...
    timeout=120
    while [ $timeout -gt 0 ]; do
        # Check the container's health status directly
...
show_logs() {
    if [ "$1" = "logs" ]; then
        ...
        docker-compose logs -f one-system-app
    fi
}
